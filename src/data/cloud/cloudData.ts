...
export async function deleteCloudTemplate(userId: string, id: string) {
  const { error } = await client().from("templates").delete().eq("user_id", userId).eq("id", id);
  if (error) throw error;
}
...
export async function loadCloudBodyweights(userId: string): Promise<BodyweightEntry[]> {
  const { data, error } = await client().from("bodyweight_entries").select("*").eq("user_id", userId).order("recorded_at", { ascending: false });
  if (error) throw error;
  return (data ?? []).map(row => ({ id: row.id, userId: row.user_id, weight: Number(row.weight), recordedAt: row.recorded_at, createdAt: row.created_at }));
}
export async function saveCloudBodyweight(userId: string, entry: BodyweightEntry) {
  const { error } = await client().from("bodyweight_entries").upsert({ id: entry.id, user_id: userId, weight: entry.weight, recorded_at: entry.recordedAt, created_at: entry.createdAt });
  if (error) throw error;
}
export async function deleteCloudBodyweight(userId: string, id: string) {
  const { error } = await client().from("bodyweight_entries").delete().eq("user_id", userId).eq("id", id);
  if (error) throw error;
}
